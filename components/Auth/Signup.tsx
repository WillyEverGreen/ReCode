...
const Signup: React.FC<SignupProps> = ({
  ...
  const handleGitHubLogin = () => {
    ...
    const state = 'github_oauth_' + Math.random().toString(36).substring(2, 10);
      ...
      <div className="w-full max-w-sm bg-[#0f1420] border border-[#1f2937] rounded-2xl p-6 relative z-10">
        ...
        {step === 'details' ? (
          <>
            ...
            <form onSubmit={handleSignup} className="space-y-3">
              <div className="relative">
                <User
                  className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${focusedInput === 'username' ? 'text-[#e6c888]' : 'text-[#4b5563]'}`}
                />
                <input
                  type="text"
                  value={username}
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="relative">
              <KeyRound
                className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${focusedInput === 'otp' ? 'text-[#e6c888]' : 'text-[#4b5563]'}`}
              />
              <input
                type="text"
                value={otp}
