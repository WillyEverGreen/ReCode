  ...
  try {
      ...
      try {
        ...
        if (user) {
          ...
          if (user.role === 'admin') {
            ...
            isAdmin = true;
            console.log(
              `[USAGE INCREMENT] ✨ Unlimited access for ADMIN user: ${user.username}`
            );
          } else if (user.plan === 'trial') {
            if (new Date() > new Date(user.trialEndDate)) {
              trialExpired = true;
              console.log(
                `[USAGE INCREMENT] ⏰ Trial expired for user: ${user.username}`
              );
            } else {
              console.log(
                `[USAGE INCREMENT] 🎯 Trial user ${user.username} - Daily limits (1 Get, 2 Analyze per day)`
              );
            }
          } else if (user.plan === 'pro') {
            console.log(
              `[USAGE INCREMENT] 💎 Pro user ${user.username} - High limits (10/10/10)`
            );
          }
        }
      } catch (err) {
        console.warn(
          `[USAGE INCREMENT] Could not fetch user details: ${err.message}`
        );
      }
    }
    ...
    if (!isDevEnv && !skipLimits) {
      ...
      if (!canContinue) {
        ...
        console.warn(
          `[USAGE INCREMENT] ❌ Limit reached for ${type}: ${JSON.stringify(limitInfo[type])}`
        );
      ...
      console.log(
        `[USAGE INCREMENT] Skipping limit enforcement for ${type} in development mode`
      );
    }
