 ...
 */
UserUsageSchema.statics.getTodayUsage = async function (
  userId,
  plan = 'trial',
  role = 'user'
) {
  ...
  const limits = isAdmin
    ? { getSolution: 999999, addSolution: 999999, variant: 999999 }
    : plan === 'pro'
      ? PRO_LIMITS
      : TRIAL_LIMITS;
  ...
  return {
    ...
    addSolutionLimit: limits.addSolution,
    variantLimit: limits.variant,
    getSolutionLeft: Math.max(
      0,
      limits.getSolution - (usage.getSolutionCount || 0)
    ),
    addSolutionLeft: Math.max(
      0,
      limits.addSolution - (usage.addSolutionCount || 0)
    ),
 ...
 * @param {string} role - User role ('user' or 'admin')
 */
UserUsageSchema.statics.canMakeRequest = async function (
  userId,
  type,
  plan = 'trial',
  role = 'user'
...
UserUsageSchema.statics.incrementUsage = async function (userId, type) {
  ...
  console.log(
    `[USAGE] Incremented ${type} for user ${userId}: ${result[field]}`
  );
  return result;
};
...
UserUsageSchema.statics.checkLimit = async function (userId, type) {
  console.warn(
    'UserUsage.checkLimit is deprecated. Use UserUsage.canMakeRequest instead.'
  );
