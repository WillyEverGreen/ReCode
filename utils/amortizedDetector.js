...
export function extractAmortizedFeatures(code, language = 'python') {
  ...
  for (const line of lines) {
    ...
    // Stack operations
    if (
      lowerLine.includes('stack.push') ||
      lowerLine.includes('.append(') ||
      lowerLine.includes('stack.add') ||
      lowerLine.includes('push(')
    ) {
      features.stackPush++;
    }
    ...
    // Queue operations
    if (
      lowerLine.includes('queue.push') ||
      lowerLine.includes('queue.add') ||
      lowerLine.includes('enqueue') ||
      lowerLine.includes('.offer(') ||
      lowerLine.includes('deque.append')
    ) {
      features.queueEnqueue++;
    }
    if (
      lowerLine.includes('queue.pop') ||
      lowerLine.includes('dequeue') ||
      lowerLine.includes('.poll()') ||
      lowerLine.includes('popleft')
    ) {
    ...
    if (
      /left\s*\+\+|left\s*\+=\s*1|l\s*\+\+|l\s*\+=\s*1|start\s*\+\+/.test(
        lowerLine
      )
    ) {
    ...
    if (
      /right\s*\+\+|right\s*\+=\s*1|r\s*\+\+|r\s*\+=\s*1|end\s*\+\+/.test(
        lowerLine
      )
    ) {
    ...
    if (
      /left\s*=\s*0|right\s*=\s*0|l\s*=\s*0|r\s*=\s*0|start\s*=\s*0/.test(
        lowerLine
      ) &&
      !lowerLine.includes('//') &&
    ...
    if (
      /while\s*\(\s*(!?\s*stack\.isempty|stack\.length|stack\.size|len\(stack\))/.test(
        lowerLine
      ) ||
    ...
    if (
      /while\s*\(\s*(!?\s*queue\.isempty|queue\.length|queue\.size|len\(queue\))/.test(
        lowerLine
      ) ||
...
export function detectAmortizedPattern(features) {
  ...
  if (
    ...
    return {
      ...
      spaceComplexity: 'O(n)',
      reason:
        'Each element is enqueued and dequeued once (BFS/level-order traversal)',
    };
  }
  ...
  if (
    ...
    return {
      ...
      spaceComplexity: 'O(1)',
      reason:
        'Both pointers move forward at most n times total (sliding window/two pointers)',
    };
  }
  ...
  if (
    ...
    return {
      ...
      spaceComplexity: 'O(k)',
      reason:
        'Sliding window with hash set - each element added/removed at most once',
    };
  }
