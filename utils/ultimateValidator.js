...
import { validateAgainstGroundTruth, applyGroundTruthCorrections } from './problemGroundTruth.js';
import { detectAlgorithmicPattern, inferComplexityFromPattern } from './patternDetector.js';
...
export async function validateComplexity(questionName, aiResponse, code, language) {
  ...
  const groundTruthResult = validateAgainstGroundTruth(questionName, aiResponse);
  ...
  const patterns = {
    bruteForce: code.bruteForce ? detectAlgorithmicPattern(code.bruteForce, language) : null,
    better: code.better ? detectAlgorithmicPattern(code.better, language) : null,
    optimal: code.optimal ? detectAlgorithmicPattern(code.optimal, language) : null
  };
  
  const patternInference = {
    bruteForce: patterns.bruteForce ? inferComplexityFromPattern(patterns.bruteForce, code.bruteForce) : null,
    better: patterns.better ? inferComplexityFromPattern(patterns.better, code.better) : null,
    optimal: patterns.optimal ? inferComplexityFromPattern(patterns.optimal, code.optimal) : null
  };
  
  if (patternInference.bruteForce || patternInference.optimal) {
    console.log('[LAYER 2] ✅ Patterns detected');
    console.log(`[LAYER 2] Confidence: ${Math.max(
  ...
  if (engineAnalysis.bruteForce?.corrected || engineAnalysis.optimal?.corrected) {
  ...
  const finalSolution = buildConsensus(validationLayers, aiResponse, questionName);
  
  console.log(`[CONSENSUS] ✅ Final confidence: ${(finalSolution.confidence * 100).toFixed(1)}%`);
      ...
      switch (layer.layer) {
        case 'groundTruth':
          if (layer.result.needsCorrection) {
            finalSolution = applyGroundTruthCorrections(finalSolution, layer.result.groundTruth);
...
function applyEngineCorrections(solution, engineResult, corrections) {
  if (engineResult.bruteForce?.corrected) {
    if (solution.bruteForce) {
      solution.bruteForce.timeComplexity = engineResult.bruteForce.timeComplexity;
      solution.bruteForce.spaceComplexity = engineResult.bruteForce.spaceComplexity;
      corrections.push({
        approach: 'bruteForce',
        field: 'complexity',
        oldValue: 'AI value',
...
function validateBetterApproach(solution, corrections) {
  ...
  if (bruteTC === optimalTC) {
    ...
    if (bruteSC === optimalSC && solution.better) {
      console.log('[VALIDATION] ⚠️ Removing invalid "better" (same TC/SC as brute/optimal)');
  ...
  if (solution.better) {
    ...
    if (betterIndex !== -1 && bruteIndex !== -1 && optimalIndex !== -1) {
      ...
      if (!(betterIndex < bruteIndex && betterIndex > optimalIndex)) {
        console.log('[VALIDATION] ⚠️ Removing invalid "better" (not between brute and optimal)');
        solution.better = null;
        corrections.push({
          approach: 'better',
          field: 'existence',
  ...
  if (solution.bruteForce && solution.optimal) {
    ...
    if (bruteIndex !== -1 && optimalIndex !== -1) {
      if (optimalIndex > bruteIndex) {
        ...
        console.log('[VALIDATION] ⚠️ Swapping brute and optimal (optimal was worse)');
