...
function loadGroundTruthDatabase() {
  if (!groundTruthDatabase) {
    try {
      ...
      const totalProblems =
        groundTruthDatabase.statistics?.total ||
        Object.keys(groundTruthDatabase.problems || {}).length;
      const withComplexity =
        groundTruthDatabase.statistics?.withComplexity || 'unknown';
      console.log(
        `[COMPLEXITY ENGINE V2] Loaded ${totalProblems} problems (${withComplexity} with TC/SC) from ground truth DB`
      );
    } catch (error) {
      console.warn(
        '[COMPLEXITY ENGINE V2] Ground truth database not found, using heuristics only'
      );
 ...
 */
export function analyzeComplexityV2(
  code,
  language = 'python',
  problemTitle = null
) {
  ...
  if (groundTruth && groundTruth.complexity && groundTruth.complexity.time) {
    console.log(
      `[COMPLEXITY ENGINE V2] Found in ground truth DB (confidence: ${groundTruth.confidence}%)`
    );
  ...
  try {
    ...
    console.warn(
      '[COMPLEXITY ENGINE V2] ⚠️ Feature extraction error:',
      err.message
    );
    features = {
      loops: { maxNestingDepth: 0 },
      dataStructures: {},
      algorithms: {},
      pointers: {},
      metrics: { dominantPattern: null },
    };
  }
  console.log('[COMPLEXITY ENGINE V2] Extracted features:', {
    ...
    dataStructures: Object.keys(features.dataStructures).filter(
      (k) => features.dataStructures[k]
    ),
  });
  ...
  if (
    codeLC.includes('merge') &&
    (codeLC.includes('mid') ||
      (codeLC.includes('left') && codeLC.includes('right'))) &&
  ...
  if (features.algorithms.backtracking) {
    if (features.algorithms.isPermutation) {
      ...
      !code.toLowerCase().includes('partition') &&
      !code.toLowerCase().includes('pivot')
    ) {
      detectedOperations.push({
  ...
  const avgExplanation = generateExplanation(
    detectedOperations,
    features,
    'average'
  );
  const worstExplanation = generateExplanation(
    detectedOperations,
    features,
    'worst'
  );
...
function calculateConfidence(operations, groundTruth) {
  ...
  const avgWeight =
    operations.reduce((sum, op) => sum + op.weight, 0) / operations.length;
