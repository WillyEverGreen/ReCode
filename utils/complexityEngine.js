...
function extractCodeFeatures(code, language = 'python') {
  ...
  const whileLoopPatterns = [/while\s*\(/gi, /while\s+\w+/gi];
  ...
  if (/\w+\s*\*\s*\w+\s*[<≤].*|Math\.sqrt/.test(code)) {
    ...
    const sqrtBound =
      /for.*(\w+)\s*\*\s*\1\s*[<≤]/.test(code) ||
      /while.*(\w+)\s*\*\s*\1\s*[<≤]/.test(code);
    if (sqrtBound) {
  ...
  features.loops.singleLoops =
    features.loops.forLoops + features.loops.whileLoops;
  ...
  function addBound(b) {
    if (!b) return;
    if (
      !/^(length|size|count|start|end|left|right|i|j|k)$/i.test(b) &&
      isNaN(b)
    ) {
  ...
  if (features.algorithms.recursionInsideLoop) {
    const isBacktracking =
      /(swap|pop|remove|false|0)\s*\(|visited.*=\s*false|\[.*,.*\]\s*=\s*\[.*,.*\]/.test(
        code
      );
  ...
  if (
    features.algorithms.recursion &&
    !features.algorithms.isPermutation &&
    !features.algorithms.divideConquer
  ) {
    const fnNames = extractFunctionNames(code, language);
    if (fnNames.length > 0) {
      // recursion with i+1
      const stepRecursion = new RegExp(
        `${fnNames[0]}\\s*\\(.*(?:\\+|\\-)\\s*1`
      ).test(code);
      if (stepRecursion) {
  ...
  if (
    features.loops.nestedLoops > 0 &&
    (features.dataStructures.stack || features.dataStructures.array)
  ) {
    const monotonic =
      /(?:for|while)[^{]*\{[\s\S]*while[^{]*\((?:.*stack|.*length|.*pop|.*peek)[^)]*\)[\s\S]*?(?:pop|shift)[\s\S]*?(?:push|add)/.test(
        code
      );
    const stackPattern =
      /while\s*\(.*stack\.length.*>.*0.*\).*(?:pop|shift)/s.test(code);
  ...
  if (
    features.metrics.recursionArgs === 'divide' &&
    features.metrics.hasRecursiveCall
  ) {
    const fnNames = extractFunctionNames(code, language);
    if (fnNames.length > 0) {
      ...
      if (occurrences >= 2) {
        features.metrics.recursionBranching = Math.max(
          features.metrics.recursionBranching,
          2
        );
      }
    }
  ...
  if (!features.dataStructures.graph) {
    ...
    if (
      hasGraphKeywords &&
      (features.dataStructures.hashMap ||
        features.dataStructures.linkedList ||
        code.includes('new List') ||
        code.includes('[]'))
    ) {
  ...
  const isBinarySearchLike = /mid\s*=.*\[.*mid.*\]|arr\[mid\]|nums\[mid\]/.test(
    code
  );
  ...
  if (features.pointers.twoPointers || features.pointers.slidingWindow) {
    ...
    const hasBackwardReset =
      backwardMovement.test(code) && code.match(/for.*for|while.*while/gi);
    if (!hasBackwardReset) {
  ...
  const dsPatterns = {
    ...
    graph: [/graph|adjacency|neighbors|edges|vertices/gi, /adj\[|adj_list/gi],
    linkedList: [
  ...
  try {
    ...
    while ((declMatch = objDeclRegex.exec(code)) !== null) {
      ...
      const indexRegex = new RegExp(
        objName + '\\s*\\[\\s*([a-zA-Z_][a-zA-Z0-9_]*)\\s*\\]\\s*=',
        'g'
      );
  ...
  const dynamicAlloc =
    /new\s+\w+\s*\[\s*[a-zA-Z_].*\]|(?:new\s+)?Array\(\s*[a-zA-Z_].*\)|\[0\]\s*\*\s*[a-zA-Z_]/.test(
      code
    );
  if (dynamicAlloc) {
  ...
  if (
    /\.slice\(|\.substring\(|\.substr\(|\.split\(|\.concat\(|new\s+String\(/.test(
      code
    )
  ) {
  ...
  try {
    ...
    while ((m = arrDeclRegex.exec(code)) !== null) {
      ...
      if (pushRegex.test(code)) {
        features.spaceUsage.auxArrays = Math.max(
          features.spaceUsage.auxArrays,
          1
        );
        features.spaceUsage.inPlace = false;
        break;
  ...
  if (features.algorithms.recursion) {
    features.metrics.recursionBranching = estimateRecursionBranching(
      code,
      functionNames
    );
  }
  ...
  if (features.algorithms.sorting && features.loops.singleLoops > 0) {
    const sortInsideLoop =
      /(for|while)[\s\S]*(\.sort|sorted|Arrays\.sort|Collections\.sort|sort\s*\()[\s\S]*(\}|:)/.test(
        code
      );
    if (sortInsideLoop) {
  ...
  if (features.loops.singleLoops > 0) {
    ...
    // Group 3: s = ... + s
    const strConcat =
      /(\w+)\s*\+=\s*['"]|(\w+)\s*=\s*\2\s*\+\s*['"]|(\w+)\s*=\s*['"][^;]*\+\s*\3/.test(
        code
      );
  ...
  if (features.algorithms.recursion && !features.algorithms.divideConquer) {
    ...
    if (fnNames.length > 0) {
      ...
      const isDandC =
        (hasMidCalc || hasSliceSplit || hasPartition) &&
  ...
  if (features.algorithms.recursion || features.algorithms.backtracking) {
    ...
    const accum =
      /\w+\.(push|add|append)\s*\(\s*(new|Arrays\.copyOf|\.\.\.|slice|\[)/.test(
        code
      );
    ...
    if (
      ...
      features.algorithms.recursion &&
      !features.algorithms.divideConquer &&
      features.metrics.recursionBranching === 1
    )
      features.algorithms.gcd = true;
  }
  ...
  if (language.toLowerCase() === 'python') {
    ...
    for (let i = 0; i < code.length; i++) {
      ...
      if (char === '}') {
        ...
        while (
          loopStack.length > 0 &&
          loopStack[loopStack.length - 1] >= braceCount
        ) {
          loopStack.pop();
        }
      }
  ...
  // B. While-loop bodies: while (i < n) { ... i *= 2 OR i /= 2 OR i = i / 2 ... }
  {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      const updateRegex = new RegExp(
        `\\b${loopVar}\\s*(\\*=|\\/=|<<=|>>=)\\s*[2-9]|\\b${loopVar}\\s*=\\s*${loopVar}\\s*\\/\\s*[2-9]|\\b${loopVar}\\s*=\\s*${loopVar}\\s*>>`
      );
...
function estimateRecursionBranching(code, functionNames) {
  ...
  for (const funcName of functionNames) {
    ...
    const defRegex = new RegExp(
      `(function|def|int|void|public|private|static)\\s+${funcName}\\s*\\(`,
      'g'
    );
    ...
    if (match) {
      ...
      const simpleDefRaw = code.match(
        new RegExp(`^\\s*${funcName}\\s*\\(`, 'm')
      );
      if (simpleDefRaw)
        startIndex = simpleDefRaw.index + simpleDefRaw[0].length;
    }
    ...
    const arithmeticBranching = new RegExp(
...
function isTrueBinarySearch(code) {
  ...
  const hasHalving =
    /\b(mid|m|middle)\b\s*=\s*.*(?:floor|trunc|>>|Math\.|2\b)/i.test(
      normalized
    );

  // 3. Check for Bounds Update (l = m + 1, etc.)
  const hasBoundUpdate =
    /\b(?:left|right|l|r|low|high|start|end)\b\s*=\s*\b(?:mid|m|middle)\b/i.test(
      normalized
    );

  // 4. Binary Keyword Fallback
  const hasBsKeyword =
    /binarySearch|binary_search|binary_exponentiation|modPow/i.test(code);
...
function estimateDPDimensions(code) {
  ...
  const twoD =
    /dp\[\s*\w+\s*\]\s*\[\s*\w+\s*\]|memo\[\s*\w+\s*\]\s*\[\s*\w+\s*\]/.test(
      code
    );
  if (twoD) return 2;
...
function determineDominantPattern(features) {
  ...
  if (features.pointers.twoPointers && features.pointers.leftRight)
    return 'two_pointers';
...
function deriveTimeComplexity(features) {
  ...
  if (features.loops.earlyExit) {
    applicableComplexities.push({
      complexity: 'O(1)',
      explanation:
        'Loop terminates immediately (unconditional break/return or sequence jump).',
      source: 'early_exit',
    });
  }
  ...
  if (features.algorithms.sortingInsideLoop && !features.loops.earlyExit) {
    applicableComplexities.push({
      ...
      explanation:
        'Sorting O(n log n) is performed inside a loop of n iterations.',
  ...
  if (
    features.dataStructures.stringConcatLoop &&
    !features.loops.earlyExit &&
    !features.dataStructures.stringBuilder
  ) {
    applicableComplexities.push({
      complexity: 'O(n²)',
      explanation:
        'Repeated string concatenation inside a loop creates O(n²) copy overhead (detected mutable string build).',
  ...
  if (features.algorithms.sieve) {
    applicableComplexities.push({
      ...
      explanation:
        'Sieve of Eratosthenes detected. This complexity relies on well-known mathematical properties (e.g., number-theoretic bounds), not just loop structure.',
      source: 'sieve',
    });
  }
  ...
  if (features.algorithms.dp) {
    // ... (existing logic)
    const explanation =
      features.metrics.dpDimensions === 2
  ...
  if (
    ...
    applicableComplexities.push({
      complexity: 'O(n)',
      explanation:
        'n Hash Table lookups/insertions (Average-case O(1); worst-case degrades with collisions).',
  ...
  if (features.loops.growthType === 'sqrt' && !features.loops.earlyExit) {
    applicableComplexities.push({
      complexity: 'O(√n)',
      explanation:
        'Loop condition (i * i < n) implies i increments until it reaches √n.',
      source: 'sqrt_loop',
    });
  }
  ...
  if (features.loops.growthType === 'logarithmic') {
    ...
    if (features.loops.nestedLoops > 0) {
      applicableComplexities.push({
        complexity: 'O(n log n)',
        explanation:
          'Inner loop runs O(log n) times (multiplicative update) inside an O(n) outer loop.',
      ...
      applicableComplexities.push({
        complexity: 'O(log n)',
        explanation:
          'Loop variable updates multiplicatively (e.g. i *= 2), taking log₂(n) steps to reach n.',
        source: 'log_loop',
      });
    }
  }
  ...
  if (
    features.loops.nestedLoops > 0 &&
    features.loops.maxNestingDepth >= 2 &&
    features.loops.growthType === 'linear'
  ) {
    if (!(features.pointers.slidingWindow && features.pointers.leftRight)) {
      ...
      if (features.algorithms.monotonicStack) {
        applicableComplexities.push({
          complexity: 'O(n)',
          explanation:
            'Monotonic Stack pattern. Although nested loops may appear, this is amortized O(n) because each pointer or element advances monotonically and is processed a bounded number of times.',
      ...
      // Check for symbolic bounds (n*m)
        ...
        const explanation =
          features.loops.maxNestingDepth >= 3
        ...
        applicableComplexities.push({
          ...
          explanation:
            explanation +
            ' Time complexity is reported for the worst-case execution path.',
          source: 'nested_loops',
        });
      }
    }
  ...
  if (features.algorithms.binarySearch) {
    ...
    const bsInsideForLoop =
      features.loops.forLoops > 0 && features.loops.whileLoops > 0;
    if (bsInsideForLoop) {
      applicableComplexities.push({
        complexity: 'O(n log n)',
        explanation:
          'Binary search O(log n) is performed inside a loop of n iterations.',
  ...
  if (
    (features.algorithms.backtracking ||
    ...
    if (features.algorithms.isPermutation) {
      applicableComplexities.push({
        complexity: 'O(n!)',
        explanation:
          'Permutation backtracking. This is a worst-case upper bound. Pruning, constraints, or early termination may reduce runtime for specific inputs.',
      ...
      applicableComplexities.push({
        complexity: 'O(k^n)',
        explanation:
          'Backtracking exploration. This is a worst-case upper bound. Pruning, constraints, or early termination may reduce runtime for specific inputs.',
  ...
  if (
    features.algorithms.recursion &&
    !features.algorithms.memoization &&
    !features.algorithms.dp
  ) {
    const branching = features.metrics.recursionBranching;
    ...
    if (features.algorithms.gcd) {
      applicableComplexities.push({
        complexity: 'O(log n)',
        explanation:
          'Euclidean algorithm. This complexity relies on well-known mathematical properties (e.g., number-theoretic bounds), not just loop structure.',
        source: 'gcd',
      });
      ...
      if (branching === 1) {
        ...
        applicableComplexities.push({
          complexity: 'O(log n)',
          explanation:
            'Recursive step divides input by constant factor with single branch.',
        ...
        if (hasLoop) {
          ...
          applicableComplexities.push({
            complexity: 'O(n log n)',
            explanation:
              'Divide and conquer with linear work at each step (e.g. Merge Sort).',
      ...
      applicableComplexities.push({
        complexity: 'O(2^n)',
        explanation:
          'Recursive calls branch into multiple paths without caching (Tree Recursion).',
  ...
  if (features.algorithms.dp || features.algorithms.memoization) {
    ...
    const is2D =
      dimensions === 2 ||
      (dimensions === 0 && features.loops.maxNestingDepth >= 2);
  ...
  if (
    (features.algorithms.bfs || features.algorithms.dfs) &&
    (features.dataStructures.graph || features.dataStructures.tree)
  ) {
    applicableComplexities.push({
      complexity: 'O(V + E)',
  ...
  if (features.pointers.slidingWindow && features.pointers.leftRight) {
    applicableComplexities.push({
      complexity: 'O(n)',
      explanation:
        'Sliding Window. Although nested loops may appear, this is amortized O(n) because each pointer or element advances monotonically and is processed a bounded number of times.',
  ...
  if (
    features.pointers.twoPointers &&
    features.pointers.leftRight &&
    !features.pointers.slidingWindow
  ) {
    applicableComplexities.push({
      complexity: 'O(n)',
      explanation:
        'Two Pointers. Although nested loops may appear, this is amortized O(n) because each pointer or element advances monotonically and is processed a bounded number of times.',
      source: 'two_pointers',
    });
  }
  ...
  if (
    (features.dataStructures.hashMap || features.dataStructures.hashSet) &&
    features.loops.nestedLoops === 0 &&
    features.loops.singleLoops > 0
  ) {
    applicableComplexities.push({
      complexity: 'O(n)',
      explanation:
        'Average-case O(1) per operation under standard hashing assumptions.',
      source: 'hash_based',
    });
  }
  ...
  if (
    ...
    applicableComplexities.push({
      complexity: 'O(n)',
      explanation:
        'Single loop iterates through n elements with constant work per iteration.',
      source: 'single_loop',
    });
  }
  ...
  applicableComplexities.sort(
    (a, b) => getComplexityRank(b.complexity) - getComplexityRank(a.complexity)
  );
  ...
  const prioritySources = [
    'divide_conquer_nlogn',
    'divide_conquer_linear',
    'divide_conquer_tree',
    'gcd',
    'graph_traversal',
    'sieve',
  ...
  const genericSources = [
    ...
    'hash_map_usage',
    'hash_based',
    'recursion_inside_loop_linear',
    'single_loop',
    'two_pointers',
  ];

  // Check if we have a priority source
  const hasPriority = applicableComplexities.find((c) =>
    prioritySources.includes(c.source)
  );
  const hasGeneric = applicableComplexities.find((c) =>
    genericSources.includes(c.source)
  );

  let dominant;
  if (hasPriority && hasGeneric) {
    // Filter out generics that might have wrongly dominated
    const priorityOnly = applicableComplexities.filter(
      (c) => !genericSources.includes(c.source)
    );
    priorityOnly.sort(
      (a, b) =>
        getComplexityRank(b.complexity) - getComplexityRank(a.complexity)
    );
  ...
  if (applicableComplexities.length > 1) {
    const others = applicableComplexities
      .slice(1)
...
function deriveSpaceComplexity(features) {
  ...
  if (features.spaceUsage.hiddenAllocations) {
    ...
    explanations.push(
      'Hidden allocations (e.g., slicing or substring creation) increase auxiliary space, even when no explicit data structure is declared'
    );
  }
  ...
  if (
    ...
    total = 'O(n²)';
    explanations.push(
      'String concatenation generates O(n²) total garbage, with O(n) peak string size'
    );
  }
  ...
  if (features.dataStructures.hashMap || features.dataStructures.hashSet) {
    ...
    total = total === 'O(n²)' ? 'O(n²)' : 'O(n)';
    explanations.push(
      'A hash map/set is used which may store up to n elements'
    );
  }
  ...
  if (features.algorithms.dp) {
    ...
    if (is2D) {
      ...
      total = 'O(n²)';
      explanations.push(
        'A 2D DP table is used with m×n dimensions (reported as O(n²) when m and n are on the same order)'
      );
    } else {
      complexity = 'O(n)';
      peak = 'O(n)';
  ...
  if (features.algorithms.accumulatesResults) {
    if (features.algorithms.isPermutation) {
      ...
      peak = 'O(n)';
      explanations.push(
        'Space complexity includes output storage, which can be exponential in the worst case for enumeration problems'
      );
    } else if (
      features.metrics.recursionArgs === 'step' ||
      features.metrics.recursionBranching >= 2
  ...
  if (features.algorithms.recursion && !features.algorithms.memoization) {
    ...
    if (
      features.algorithms.binarySearch ||
      features.algorithms.divideConquer ||
      features.metrics.recursionArgs === 'divide'
    ) {
      ...
      explanations.push(
        'Recursive calls use O(log n) stack space due to divide and conquer'
      );
    } else {
  ...
  if (
    ...
    explanations.push(
      'Space is bounded by the distinct keys in the window (e.g. alphabet size k), treated as O(k).'
    );
  }
  ...
  if (features.spaceUsage.inPlace && explanations.length === 0) {
    complexity = 'O(1)';
    explanations.push(
      'The algorithm operates in-place using only constant extra space'
    );
  }
...
function verifyComplexity(analysisResult, features) {
  ...
  const isExponential = (tc) =>
    tc.includes('2^n') || tc.includes('k^n') || tc.includes('!');
  const isLogLinearOrQuadratic = (tc) =>
    tc.includes('n log n') || tc.includes('n^2') || tc.includes('n²');
  ...
  if (f.algorithms.sorting || f.algorithms.sortingInsideLoop) {
    if (isExponential(timeComplexity)) {
      ...
      correctedReason =
        'Correction: Sorting algorithms cannot be exponential. Standard sorting is O(n log n).';
    }
  }

  // 2️⃣ SEARCHING ALGORITHMS (Binary Search, BFS, DFS)
  if (
    f.algorithms.binarySearch ||
    (f.algorithms.bfs && !f.dataStructures.graph) ||
    (f.algorithms.dfs && !f.dataStructures.graph)
  ) {
    if (isExponential(timeComplexity)) {
      correctedTC = f.algorithms.binarySearch ? 'O(log n)' : 'O(n)';
      correctedReason =
        'Correction: Search algorithms on standard structures cannot be exponential.';
    }
  }
  ...
  if (f.algorithms.divideConquer) {
    if (isExponential(timeComplexity) && !f.algorithms.accumulatesResults) {
      ...
      correctedReason =
        'Correction: Divide & Conquer algorithms (like QuickSort/MergeSort) are typically O(n log n), not exponential.';
    }
  }
  ...
  if (isExponential(timeComplexity)) {
    const isValidBacktracking =
      ...
      (f.algorithms.recursion &&
        (f.metrics.recursionArgs === 'step' ||
          f.metrics.recursionBranching >= 2));
    ...
    if (!isValidBacktracking && !f.algorithms.backtracking) {
      ...
      correctedReason =
        'Correction: Exponential complexity detected without backtracking structure. Re-evaluating as polynomial.';
    }
  }
  ...
  if (f.algorithms.dp || f.algorithms.memoization) {
    if (isExponential(timeComplexity)) {
      ...
      correctedReason =
        'Correction: Dynamic Programming optimizes exponential problems to polynomial time.';
    }
  }
  ...
  if (f.dataStructures.graph && (f.algorithms.bfs || f.algorithms.dfs)) {
    if (isExponential(timeComplexity)) {
      correctedTC = 'O(V + E)';
      correctedReason =
        'Correction: Graph traversal (BFS/DFS) is linear with respect to vertices and edges.';
    }
  }
  ...
  if (f.pointers.slidingWindow && timeComplexity !== 'O(n)') {
    correctedTC = 'O(n)';
    correctedReason =
      'Correction: Sliding Window pattern guarantees linear time complexity.';
  }

  if (
    f.pointers.twoPointers &&
    f.pointers.leftRight &&
    !f.pointers.slidingWindow &&
    timeComplexity !== 'O(n)'
  ) {
    correctedTC = 'O(n)';
    correctedReason =
      'Correction: Two Pointers pattern guarantees linear time complexity.';
  }
...
function checkHazardPatterns(code) {
  ...
  const isRecursiveBinarySearch =
    /function\s+\w*(binarysearch|bsearch)\w*\s*\(|def\s+\w*(binary_search|bsearch)\w*\s*\(/i.test(
      code
    ) &&
    /mid\s*=/.test(code) &&
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const hasFourSumMarker = /foursum|four_sum|4sum|nums\.length\s*-\s*3/i.test(
    code
  );
  ...
  const isExpandAroundCenter =
    (/expand/i.test(code) &&
      /while.*l.*r|while.*left.*right/i.test(code) &&
      /s\[l\].*s\[r\]|s\[left\].*s\[right\]/i.test(code)) ||
    (/longestpalindrome|longest_palindrome/i.test(code) &&
      /for\s*\(/i.test(code) &&
      /while/i.test(code));
  ...
  const isNQueens =
    ...
    (/queen/i.test(code) &&
      /backtrack/i.test(code) &&
      /isvalid|is_valid|canplace|can_place/i.test(code)) ||
    (/board/i.test(code) &&
      /row/i.test(code) &&
      /col/i.test(code) &&
      /backtrack/i.test(code));
  ...
  const isWordBreak =
    ...
    (/dp\[/i.test(code) &&
      /\.substring|\.slice|\.includes/i.test(code) &&
      /worddict|word_dict|dictionary/i.test(code));
  ...
  const isMergeKSorted =
    ...
    (/heap|priorityqueue|minheap/i.test(code) &&
      /lists|k\s*sorted/i.test(code));
  ...
  const isDijkstra =
    ...
    (/shortest|distance/i.test(code) &&
      /heap|priorityqueue/i.test(code) &&
      /relax|dist\[/i.test(code));
  ...
  const isKMP =
    ...
    (/(\blps\b|failure|prefix)/i.test(code) &&
      /(pattern|text|needle|haystack|\bhay\b|\bneed\b)/i.test(code));
  ...
  const isMonotonicStack =
    ...
    (/for\s*\([^)]*\)\s*\{[\s\S]*while\s*\([^)]*\.length[^)]*\)[\s\S]*\.pop\s*\(/i.test(
      code
    ) &&
      !/dailytemperatures|daily_temperatures|maxslidingwindow|max_sliding_window/i.test(
        code
      ));
  ...
  const isDailyTemperatures =
    ...
    (/res\s*=\s*Array\([^)]*\)\.fill\(0\)/i.test(code) &&
      /while\s*\([^)]*\.length[^)]*\)[\s\S]*res\[/i.test(code));
  ...
  const isTrie =
    ...
    (/children|child/i.test(code) &&
      /insert|search|startswith/i.test(code) &&
      /node|curr|current/i.test(code));
  ...
  const isBitCount =
    ...
    (/countbits|count_bits|hammingweight|hamming_weight|popcount|bitcount/i.test(
      code
    ) &&
      /while\s*\(\s*n\s*>\s*0/.test(code)) ||
    // Or we structurally detect a while loop that repeatedly shifts n and checks lowest bit
    (/while\s*\(\s*n\s*>\s*0/.test(code) &&
      /n\s*&\s*1/.test(code) &&
      /n\s*>>=?\s*1|n\s*=\s*n\s*>>/.test(code));
  ...
  if (isBitCount) {
    ...
    return buildHazardResult(
      isFixed ? 'O(1)' : 'O(log n)',
      isFixed
        ? 'Bit Count with fixed 32/64 bits loop is O(1).'
        : 'Bit Count / Hamming Weight. Processes bits of n, which is O(log n).',
      'O(1)',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const hasFloydWarshallMarker =
    /floydwarshall|floyd_warshall|warshall|dist\[i\]\[k\]/i.test(code);
  const isFloydCycle =
    !hasFloydWarshallMarker &&
    (/findduplicate|find_duplicate|detectcycle|detect_cycle/i.test(code) ||
      (/slow\s*=\s*\w+\[\s*slow\s*\]/.test(code) &&
        /fast\s*=\s*\w+\[\s*\w+\[\s*fast\s*\]\s*\]/.test(code)));
  ...
  const isTreeSerialization =
    ...
    (/root\.val|node\.val/.test(code) ||
      /root\.left|root\.right|node\.left|node\.right/.test(code));
  ...
  const isLRUCache =
    ...
    (/capacity/i.test(code) &&
      /cache\.get|cache\.set|cache\.delete|cache\.has/.test(code));
  ...
  const isBinaryConversion =
    ...
    (/while\s*\(\s*n\s*>\s*0/.test(code) &&
      /n\s*%\s*2/.test(code) &&
      /n\s*=.*Math\.floor.*\/\s*2|n\s*=\s*n\s*\/\s*2|n\s*\/=\s*2/.test(code));
  ...
  const isCourseSchedule =
    /courseschedule|course_schedule|canfinish|can_finish|toposort|topo_sort/i.test(
      code
    ) ||
    (/indegree|in_degree|indeg/i.test(code) &&
      /(prerequisites|prereq|edges|adj)/i.test(code));
  ...
  const isCloneGraph =
    ...
    (/clone|visited/i.test(code) &&
      /neighbors/i.test(code) &&
      /dfs|bfs/i.test(code));
  ...
  const isWordLadder =
    ...
    (/beginword|begin_word|endword|end_word/i.test(code) &&
  ...
  const isMedianArrays =
    ...
    (/nums1|nums2/i.test(code) &&
      /maxleft|minright|infinity/i.test(code) &&
      /while.*left.*right/i.test(code));
  ...
  const isFloydWarshall =
    ...
    (/for.*k.*for.*i.*for.*j|for.*for.*for/.test(code) &&
      /dist\[i\]\[k\].*dist\[k\]\[j\]|graph\[i\]\[k\].*graph\[k\]\[j\]/.test(
        code
      ));
  ...
  const isTreeTraversal =
    /inverttree|invert_tree|maxdepth|max_depth|mindepth|min_depth|isvalidbst|is_valid_bst|validatebst|lowestcommonancestor|lca/i.test(
      code
    ) ||
    (/root\.left|root\.right/.test(code) &&
      /return.*root|return.*node/.test(code) &&
      !/serialize|deserialize|floyd|dist\[/i.test(code));
  ...
  const isLevelOrder =
    ...
    (/queue/i.test(code) &&
      /node\.left|node\.right|root\.left|root\.right/.test(code) &&
      /shift|dequeue/.test(code));
  ...
  const isMergeSorted =
    ...
    (/arr1|arr2|nums1|nums2/i.test(code) &&
      /while.*i.*j|while.*&&/.test(code) &&
      /push|result/.test(code) &&
      !/median/i.test(code));
  ...
  const isAnagram =
    ...
    (/isanagram|is_anagram|validanagram|valid_anagram/i.test(code) &&
      /\.sort\s*\(/.test(code)) ||
  ...
  const isSpiralMatrix =
    ...
    (/top.*bottom.*left.*right/i.test(code) &&
      /while.*top.*bottom|while.*left.*right/.test(code));
  ...
  const isFourSum =
    ...
    (/\.sort/.test(code) &&
      /for.*for.*while.*left.*right|for.*for.*left.*right/.test(code));
  ...
  const isEditDistance =
    /editdistance|edit_distance|mindistance|min_distance|levenshtein/i.test(
      code
    ) ||
    (/word1|word2|text1|text2/i.test(code) &&
      /dp\[i\]\[j\]|dp\[i-1\]\[j-1\]/.test(code));
  ...
  const isLCS =
    ...
    (/const\s+dp\s*=\s*Array\.from\(/i.test(code) &&
      /dp\[i\]\[j\]/i.test(code) &&
      /a\[i-1\]\s*===\s*b\[j-1\]/i.test(code));
  ...
  const isWordSearch =
    ...
    (/board|grid/i.test(code) &&
      /word\[k\]|word\[index\]/i.test(code) &&
      /backtrack|dfs/i.test(code));
  ...
  const isLongestConsecutive =
    ...
    (/set\.has\(num\s*-\s*1\)|!set\.has\(num\s*-\s*1\)/.test(code) &&
      /while.*set\.has/.test(code));
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const isLISNLogN =
    /lengthoflis|longestincreasingsubsequence|longest_increasing_subsequence/i.test(
      code
    ) ||
  ...
  const isTopKFrequent =
    ...
    (/freq|frequency|count/i.test(code) &&
      /\.sort\s*\(/i.test(code) &&
      /(slice\(0,\s*k\)|slice\(\s*-k\))/i.test(code));
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const hasSievePattern =
    /sieve|primes?\[|j\s*\+=\s*i|j\s*=\s*i\s*\*\s*i/i.test(code);
  const isSqrtLoop =
    !hasSievePattern &&
    (/mysqrt|my_sqrt|isqrt|intsqrt|int_sqrt|squareroot|square_root/i.test(
      code
    ) ||
      /for\s*\([^)]*i\s*\*\s*i\s*<=\s*n|while\s*\([^)]*i\s*\*\s*i\s*<=\s*n/.test(
        code
      ));
  ...
  const isReverseWords =
    ...
    (/\.trim\s*\(\s*\)\s*\.split\s*\(/i.test(code) &&
      /\.reverse\s*\(\s*\)\s*\.join\s*\(/i.test(code));
  ...
  const isGroupAnagrams =
    ...
    (/new\s+Map\s*\(/i.test(code) &&
      /\.split\('\'\)\.sort\s*\(\)\.join\('\'\)/i.test(code));
  ...
  const isRotateArrayByK =
    /rotate\s*\(/i.test(code) &&
    /function\s+rotate\s*\(/i.test(code) &&
    /function\s+rev\s*\(/i.test(code);
  ...
  const isNextPermutation =
    ...
    (/while\s*\(i>=0\s*&&\s*\w+\[i]\s*>=\s*\w+\[i\+1]/i.test(code) &&
      /while\s*\(l<r\)/i.test(code));
  ...
  const isGenerateParentheses =
    /generateparenthesis|generate_parenthesis|generateparentheses|generate_parentheses/i.test(
      code
    );
  ...
  const isCoinChangeMinCoins =
    /coinchange|coin_change/i.test(code) &&
    /dp\s*=\s*Array\(amount\+1\)/i.test(code);
  ...
  const isRemoveNth =
    ...
    (/let\s+fast\s*=\s*dummy,\s*slow\s*=\s*dummy/i.test(code) &&
      /for\s*\(let i=0;i<n;i\+\+\)/i.test(code));
  ...
  const isSlidingWindowMax =
    ...
    (/const\s+dq\s*=\s*\[\s*\]/i.test(code) &&
      /while\s*\([^)]*dq\.length[^)]*\)[\s\S]*dq\.pop\s*\(/i.test(code) &&
      /dq\.shift\s*\(/i.test(code));
  ...
  const isNumberOfIslands =
    ...
    (/grid/i.test(code) &&
      /dfs\s*\(/i.test(code) &&
      /i\+1,j|i-1,j|i,j\+1|i,j-1/.test(code));
  ...
  const isFirstUnique =
    ...
    (/count|freq/i.test(code) &&
      /charcodeat|charcode/i.test(code) &&
      /===\s*1|==\s*1/.test(code));
  ...
  const isSetMatrixZeroes =
    /setzeroes|set_zeroes|setzeros|set_zeros|matrixzeroes|matrix_zeroes/i.test(
      code
    ) ||
    (/firstrowzero|firstcolzero|first_row_zero|first_col_zero/i.test(code) &&
      /matrix\[i\]\[0\]|matrix\[0\]\[j\]/.test(code));
  ...
  const isFourSumImproved =
    /foursum|four_sum|4sum/i.test(code) ||
    (/nums\.length\s*-\s*3/.test(code) &&
      /nums\.length\s*-\s*2/.test(code) &&
      /left.*right|lo.*hi/.test(code)) ||
    (/for.*for.*while.*left.*right/s.test(code) &&
      /nums\[i\].*nums\[j\].*nums\[left\].*nums\[right\]|i.*j.*left.*right/.test(
        code
      ));
...
function calculateConfidence(features) {
  ...
  if (features.algorithms.recursion && !features.algorithms.memoization) {
    ...
    confidence = features.metrics.recursionArgs === 'divide' ? 0.85 : 0.7;
  }
...
function validateComplexity(aiTC, aiSC, code, language = 'python') {
  ...
  const normalizedSystemSC = normalizeComplexity(
    systemAnalysis.spaceComplexity
  );
  ...
  if (
    systemAnalysis.pattern === 'sliding_window' &&
  ...
  if (
    systemAnalysis.pattern === 'two_pointers' &&
    normalizedAiTC.includes('n²')
  ) {
    criticalErrors.push({
      type: 'TIME_COMPLEXITY',
  ...
  if (
    systemAnalysis.features.algorithms.sorting &&
  ...
  const isConstantSpaceClaim =
    normalizedAiSC === 'O(1)' || normalizedAiSC === 'O(0)';
  ...
  const hasDetectedSpace =
    ...
    (systemAnalysis.features.algorithms.recursion &&
      !systemAnalysis.features.pointers.tailRecursion) || // Tail recursion optimization is rare in JS/Java standard engines
  ...
  if (isConstantSpaceClaim && hasDetectedSpace) {
    ...
    if (!systemIsConstant) {
      criticalErrors.push({
        ...
        error:
          'Auxiliary space usage ignored (Stack/Queue/Recursion/Map detected)',
        aiValue: aiSC,
  ...
  const isDeterministic = deterministicPatterns.includes(
    systemAnalysis.pattern
  );
  ...
  const shouldOverride =
    criticalErrors.length > 0 || (isDeterministic && differentResult);
...
function getCorrectedComplexity(aiTC, aiSC, code, language = 'python') {
  ...
  if (validation.shouldOverride) {
    console.log(
      '[COMPLEXITY ENGINE] ✓ Overriding AI complexity with system analysis'
    );
    console.log(
      `[COMPLEXITY ENGINE] Pattern: ${validation.systemAnalysis.pattern}`
    );
    console.log(
      `[COMPLEXITY ENGINE] TC: ${aiTC} → ${validation.systemAnalysis.timeComplexity}`
    );
    console.log(
      `[COMPLEXITY ENGINE] SC: ${aiSC} → ${validation.systemAnalysis.spaceComplexity}`
    );
