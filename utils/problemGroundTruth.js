...
export const PROBLEM_GROUND_TRUTH = {
  ...
  'anagram': {
    ...
    better: {
      ...
      name: 'Sorting',
      scReason: 'O(n) space is required to convert immutable Strings into sortable Character Arrays.'
    },
    optimal: {
      tc: 'O(n)',
      sc: 'O(1)',
  ...
  'maxsubarraysum': {
    patterns: ['maximum subarray', 'max subarray sum', 'kadane', 'kadanes algorithm'],
    bruteForce: {
      tc: 'O(n³)',
      sc: 'O(1)',
  ...
  'longestconsecutivesequence': {
    patterns: ['longest consecutive', 'consecutive sequence', 'longest consecutive sequence'],
    bruteForce: {
      tc: 'O(n²)',
      sc: 'O(1)',
  ...
  'mergeintervals': {
    patterns: ['merge intervals', 'merge overlapping intervals', 'overlapping intervals'],
    bruteForce: {
      tc: 'O(n²)',
      sc: 'O(n)',
  ...
  'longestsubstringwithoutrepeating': {
    patterns: ['longest substring without repeating', 'longest substring', 'no repeating characters'],
    bruteForce: {
      tc: 'O(n³)',
  ...
  'validparentheses': {
    patterns: ['valid parentheses', 'balanced parentheses', 'parentheses matching'],
    bruteForce: {
      tc: 'O(n)',
      sc: 'O(n)',
  ...
  'sieveoferatosthenes': {
    patterns: ['sieve of eratosthenes', 'sieve', 'all primes', 'primes up to n'],
    bruteForce: {
      tc: 'O(n² log log n)',
      sc: 'O(1)',
  ...
  'wordsearch': {
    ...
    bruteForce: {
      ...
      name: 'Backtracking',
      tcReason: 'N cells * 3^L checks (branching factor 3) where L is word length.'
    },
    better: null,
    optimal: {
      tc: 'O(N * 3^L)',
  ...
  'uniquepathsiii': {
    patterns: ['unique paths iii', 'unique paths 3', 'walk over every empty square'],
    bruteForce: {
      tc: 'O(3^N)',
      sc: 'O(N)',
  ...
  'medianfinder': {
    patterns: ['median finder', 'find median', 'running median', 'median from data stream'],
    bruteForce: {
      tc: 'O(n log n)',
      sc: 'O(n)',
  ...
  'kmp': {
    patterns: ['kmp', 'knuth morris pratt', 'pattern matching', 'string matching'],
    bruteForce: {
      tc: 'O(n*m)',
      sc: 'O(1)',
  ...
  'singlenumberii': {
    patterns: ['single number ii', 'single number 2', 'appears once others thrice'],
    bruteForce: {
      tc: 'O(n)',
      sc: 'O(n)',
...
export function validateAgainstGroundTruth(problemName, aiSolution) {
  ...
  if (aiSolution.bruteForce && groundTruth.bruteForce) {
    const aiTC = normalizeProblemName(aiSolution.bruteForce.timeComplexity || '');
  ...
  if ((groundTruth.better === null || groundTruth.hasOptimizationLadder === false) && aiSolution.better !== null) {
    corrections.push({
      approach: 'better',
      field: 'existence',
      aiValue: 'exists',
      correctValue: 'should be null',
      reason: groundTruth.hasOptimizationLadder === false 
        ? 'This problem does not have an optimization ladder.' 
        : (groundTruth.note || 'No intermediate approach exists for this problem')
    });
    needsCorrection = true;
  }
...
export function applyGroundTruthCorrections(aiSolution, groundTruth) {
  ...
  if (groundTruth.bruteForce) {
    ...
    corrected.bruteForce.timeComplexityReason = generateReason(groundTruth.bruteForce.name, groundTruth.bruteForce.tc, 'time', groundTruth.bruteForce.tcReason);
    corrected.bruteForce.spaceComplexityReason = generateReason(groundTruth.bruteForce.name, groundTruth.bruteForce.sc, 'space', groundTruth.bruteForce.scReason);
  ...
  if (groundTruth.better === null || groundTruth.hasOptimizationLadder === false) {
    ...
    corrected.better.timeComplexityReason = generateReason(groundTruth.better.name, groundTruth.better.tc, 'time', groundTruth.better.tcReason);
    corrected.better.spaceComplexityReason = generateReason(groundTruth.better.name, groundTruth.better.sc, 'space', groundTruth.better.scReason);
  ...
  if (groundTruth.optimal && corrected.optimal) {
    ...
    // FORCE overwrite reasons
    corrected.optimal.timeComplexityReason = generateReason(groundTruth.optimal.name, groundTruth.optimal.tc, 'time', groundTruth.optimal.tcReason);
    corrected.optimal.spaceComplexityReason = generateReason(groundTruth.optimal.name, groundTruth.optimal.sc, 'space', groundTruth.optimal.scReason);
