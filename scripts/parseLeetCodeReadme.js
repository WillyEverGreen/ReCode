...
const TABLE_ROW_REGEX = /^\|\s*(\d+)\s*\|\s*\[([^\]]+)\]\(([^)]+)\)\s*\|.*?\|\s*_([^_]+)_\s*\|\s*_([^_]+)_\s*\|\s*(\w+)/gm;
...
function parseProblemLine(line) {
  ...
  return {
    ...
    tags: tags.split(/[,\s]+/).map(t => t.trim()).filter(t => t.length > 0)
  };
}
...
function parseComplexityColumn(text) {
  ...
  if (defaultMatch) {
    ...
    result.default = result.runtime || result[Object.keys(result)[Object.keys(result).length - 1]];
  }
...
function parseLeetCodeReadme() {
  ...
  console.log(`✅ Successfully parsed ${problems.length} problems with TC/SC\n`);
  ...
  problems.forEach(p => {
    ...
    if (tc.includes('log')) stats.byComplexityClass['logarithmic'] = (stats.byComplexityClass['logarithmic'] || 0) + 1;
    else if (tc.includes('n²') || tc.includes('n^2')) stats.byComplexityClass['quadratic'] = (stats.byComplexityClass['quadratic'] || 0) + 1;
    else if (tc.includes('2^n')) stats.byComplexityClass['exponential'] = (stats.byComplexityClass['exponential'] || 0) + 1;
    else if (tc.includes('n!')) stats.byComplexityClass['factorial'] = (stats.byComplexityClass['factorial'] || 0) + 1;
    else if (tc.includes('n') && !tc.includes('log')) stats.byComplexityClass['linear'] = (stats.byComplexityClass['linear'] || 0) + 1;
    else if (tc.includes('1')) stats.byComplexityClass['constant'] = (stats.byComplexityClass['constant'] || 0) + 1;
  });
    ...
    problems: problems.reduce((acc, p) => {
      ...
      acc[id] = {
        ...
        complexity: {
          time: timeAvgWorst,
          space: spaceAvgWorst,
          hasDistinctCases: (timeAvgWorst.average !== timeAvgWorst.worst || spaceAvgWorst.average !== spaceAvgWorst.worst)
        },
        verified: true,
