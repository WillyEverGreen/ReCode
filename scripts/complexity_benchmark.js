...
const runBenchmark = () => {
    ...
    for (const [key, problem] of Object.entries(problems)) {
        ...
        if (problem.code.trim().startsWith('//') && problem.code.split('\n').length < 5) continue;

        totalWithCode++;

        // Get Expected Complexity
        const expectedTC = problem.complexity?.time?.worst || problem.complexity?.time?.average;
        const expectedSC = problem.complexity?.space?.worst || problem.complexity?.space?.average;
        ...
        // Fuzzy match logic
        const isMatch = normPredicted.includes(normExpected) || normExpected.includes(normPredicted);
        ...
        if (isMatch) {
            ...
            mismatches.push({
                ...
                reason: analysis.patterns.length > 0 ? `Pattern: ${analysis.patterns[0]}` : "Loop Analysis"
            });
        }
    }
    ...
    log(`Accuracy Rate:            ${((matchCount / totalWithCode) * 100).toFixed(2)}%`);

    if (mismatches.length > 0) {
        log("\nSample Mismatches (First 20):");
        mismatches.slice(0, 20).forEach(m => {
            log(`- ${m.title}: Expected '${m.expected}' (${normalize(m.expected)}), Got '${m.predicted}' (${normalize(m.predicted)}) [${m.reason}]`);
        });
    }

    fs.writeFileSync(path.join(__dirname, '../benchmark_debug.txt'), output, 'utf-8');
};
