...
const REPO_API = 'https://api.github.com/repos/arindal1/SDE-DSA-SD-Prep/git/trees/main?recursive=1';
const RAW_BASE = 'https://raw.githubusercontent.com/arindal1/SDE-DSA-SD-Prep/main/';
...
function extractComplexity(mdContent, problemName) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((line.includes('better') || line.includes('optimized')) && !line.includes('optimal')) {
...
function generatePatterns(problemName) {
  ...
  if (base.includes('linked list')) patterns.push(base.replace('linked list', 'll'));
...
async function extractAllProblems() {
  console.log('🚀 Starting ground truth extraction from Striver\'s A2Z DSA Sheet...\n');
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        const problemName = filename.replace(/\d+\./g, '').replace(/\.md$/i, '').replace(/([A-Z])/g, ' $1').trim();
    ...
    console.log(`   Coverage: ${((successful / mdFiles.length) * 100).toFixed(1)}%\n`);
