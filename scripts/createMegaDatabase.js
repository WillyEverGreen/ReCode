...
const LEETCODE_FULL_PATH = path.join(
  __dirname,
...
console.log(
  `   LeetCode Full:   ${Object.keys(leetcodeData.problems || {}).length} problems\n`
);
...
Object.entries(striverData.problems || {}).forEach(([id, problem]) => {
  if (mergedProblems[id]) {
    ...
    if (
      !problem.complexity?.time?.average &&
      mergedProblems[id].complexity?.time?.average
    ) {
...
// Statistics
const withComplexity = Object.values(mergedProblems).filter(
  (p) => p.complexity?.time?.average
).length;
...
const megaDatabase = {
  ...
  statistics: {
    ...
    withComplexity,
    coveragePercent: (
      (withComplexity / Object.keys(mergedProblems).length) *
      100
    ).toFixed(1),
...
console.log(
  `   With TC/SC Complexity: ${megaDatabase.statistics.withComplexity}`
);
console.log(`   Coverage: ${megaDatabase.statistics.coveragePercent}%`);
console.log(
  `   From Striver (priority): ${megaDatabase.statistics.fromStriver}`
);
