...
const README_DB_PATH = path.join(
  __dirname,
  '../utils/leetcodeGroundTruth.json'
);
const OUTPUT_PATH = path.join(
  __dirname,
  '../utils/leetcodeGroundTruthFull.json'
);
...
try {
  ...
  console.log(
    `📋 Loaded ${Object.keys(readmeData).length} problems from README database\n`
  );
} catch (error) {
...
for (const file of pythonFiles) {
  ...
  try {
    ...
    const timeMatch =
      content.match(/Time.*?O\(([^)]+)\)/i) ||
      content.match(/Time:\s*_?O\(([^)_]+)\)_?/i);
    const spaceMatch =
      content.match(/Space.*?O\(([^)]+)\)/i) ||
      content.match(/Space:\s*_?O\(([^)_]+)\)_?/i);
    ...
    if (readmeData[problemId]) {
      ...
      problems[problemId] = {
        ...
        complexity: {
          time: {
            average:
              timeComplexity || readmeProb.complexity?.time?.average || null,
            worst: timeComplexity || readmeProb.complexity?.time?.worst || null,
          },
          space: {
            average:
              spaceComplexity || readmeProb.complexity?.space?.average || null,
            worst:
              spaceComplexity || readmeProb.complexity?.space?.worst || null,
          },
        },
    ...
    if (parsed % 500 === 0) {
      console.log(
        `   ✓ Processed ${parsed}/${pythonFiles.length} files... (${withComplexity} with complexity)`
      );
    }
  } catch (error) {
