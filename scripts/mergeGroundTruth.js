...
const STRIVER_DB_PATH = path.join(__dirname, '../utils/groundTruthDatabase.json');
const LEETCODE_DB_PATH = path.join(__dirname, '../utils/leetcodeGroundTruth.json');
...
console.log(`   LeetCode Repo:   ${Object.keys(leetcodeData.problems || {}).length} problems\n`);
...
console.log(`   From Striver (priority): ${mergedDatabase.statistics.fromStriver}`);
console.log(`   From LeetCode: ${mergedDatabase.statistics.fromLeetCode}`);
console.log(`   Overwrites (Striver priority): ${mergedDatabase.statistics.overwritten}`);
console.log(`\n💾 Saved to: ${MERGED_DB_PATH}\n`);

// Calculate coverage estimate
const totalLeetCodeProblems = 3792; // As of README
const coveragePercent = ((mergedDatabase.statistics.total / totalLeetCodeProblems) * 100).toFixed(1);

console.log(`📈 Coverage:`);
console.log(`   ${coveragePercent}% of all LeetCode problems (${mergedDatabase.statistics.total}/${totalLeetCodeProblems})`);
