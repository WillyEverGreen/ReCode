...
function applyAmortizedOverrides(code: string, detectedTC: string): string {
  ...
  if (
    (c.includes('left++') || c.includes('l++') || c.includes('left+=')) &&
...
function detectRecursiveExplosion(code: string): boolean {
  ...
  const fnMatch = code.match(
    /def\s+(\w+)|function\s+(\w+)|(\w+)\s*=\s*\(|public\s+\w+\s+(\w+)\s*\(/
  );
...
const mapQubridError = (error: any): Error => {
  ...
  if (message.includes('Daily limit reached')) {
    return new Error(
      "You've hit today's Free plan limit. Upgrade to Pro to continue using this feature today."
    );
  }
  ...
  if (
    ...
    return new Error(
      'AI usage limit reached. Please wait a minute and try again, or upgrade to Pro if this keeps happening.'
    );
  }
...
const generatePrompt = (data: SubmissionData): string => {
  ...
  if (!cleanCode || cleanCode.trim().length < 5) {
    console.warn(
      '[SafeGuard] Comment stripping resulted in empty code. Using original.'
    );
    cleanCode = data.code;
  }
...
Return ONLY valid JSON, no markdown fences.`;
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a complexity analysis expert. When reconsidering, be honest about mistakes but also defend correct analysis. Prioritize accuracy over agreement.',
          },
    ...
    if (
      !parsed.finalTimeComplexity ||
      !parsed.finalSpaceComplexity ||
      !parsed.reasoning
    ) {
...
export const analyzeSubmission = async (
  ...
  if (data.code.length > LIMITS.code) {
    throw new Error(
      `Code too long. Maximum ${LIMITS.code} characters allowed (yours: ${data.code.length}).`
    );
  }

  if (data.problemUrl && data.problemUrl.length > LIMITS.problemUrl) {
    throw new Error(
      `Problem URL too long. Maximum ${LIMITS.problemUrl} characters allowed.`
    );
  }
    ...
    try {
      ...
      if (usageRes.ok) {
        ...
        if (usageData.addSolution?.left === 0) {
          throw new Error(
            `Daily limit reached (${usageData.addSolution.limit}/day). Try again tomorrow or upgrade to Pro!`
          );
        }
      }
    } catch (limitError) {
      ...
      if (
        ...
        limitError.message.includes('Daily limit')
      ) {
        throw limitError;
      }
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (result.improvementMarkdown) {
          result.improvementMarkdown = result.improvementMarkdown.replace(
            /```typescript\s*```typescript/g,
            '```typescript'
          );
          result.improvementMarkdown = result.improvementMarkdown.replace(
            /```\s*```/g,
        ...
        if (!result.criticalError) {
          try {
            ...
            const amortizedTC = applyAmortizedOverrides(
              data.code,
              corrected.timeComplexity
            );
            if (amortizedTC !== corrected.timeComplexity) {
              console.log(
                `[LAYER 2] Amortized override: ${corrected.timeComplexity} → ${amortizedTC}`
              );
            ...
            if (detectRecursiveExplosion(data.code)) {
              ...
              corrected.timeComplexityReason =
                'Tree recursion with multiple recursive calls per invocation';
              corrected.corrected = true;
            }
            ...
            if (forcedSC && forcedSC !== result.spaceComplexity) {
              console.log(
                `[LAYER 4] Space truth enforced: ${result.spaceComplexity} → ${forcedSC}`
              );
              corrected.spaceComplexity = forcedSC;
              corrected.spaceComplexityReason =
                'Detected auxiliary data structures or recursion stack';
              corrected.corrected = true;
            }

            if (corrected.corrected) {
              console.log(
                `[COMPLEXITY ENGINE] Detected mismatch: TC=${result.timeComplexity} → ${corrected.timeComplexity}, SC=${result.spaceComplexity} → ${corrected.spaceComplexity}`
              );
              ...
              if (reconsideration) {
                ...
                if (llmFinalTC === originalTC && llmFinalSC === originalSC) {
                  ...
                  console.log(
                    `[LLM RECONSIDER] Trusting LLM's decision: ${reconsideration.finalTC}/${reconsideration.finalSC}`
                  );
                  ...
                  console.log(
                    `[LLM RECONSIDER] LLM agreed with engine: ${corrected.timeComplexity}/${corrected.spaceComplexity}`
                  );
                  ...
                  console.log(
                    `[LLM RECONSIDER] LLM provided new analysis: ${reconsideration.finalTC}/${reconsideration.finalSC}`
                  );
                  result.timeComplexity = reconsideration.finalTC;
                  result.spaceComplexity = reconsideration.finalSC;
                  result.timeComplexityReason = reconsideration.reasoning;
                ...
                if (corrected.spaceComplexityReason)
                  result.spaceComplexityReason =
                    corrected.spaceComplexityReason;
              }
            } else {
              console.log(
                `[COMPLEXITY ENGINE] SOURCE=LLM   | Using AI-provided TC/SC without override (TC=${result.timeComplexity}, SC=${result.spaceComplexity})`
              );
            }

            // LAYER 6: Pattern → Complexity Hard Map (FINAL AUTHORITY)
            // This MUST be the absolute last complexity decision
            const coreLogicObj = result.coreLogic as
              | Record<string, any>
              | undefined;
            if (
              coreLogicObj &&
              typeof coreLogicObj === 'object' &&
              coreLogicObj.Pattern
            ) {
              const finalTC = applyPatternOverride(
                coreLogicObj.Pattern,
                result.timeComplexity
              );
            ...
            console.warn(
              '[COMPLEXITY ENGINE] Error (using AI values):',
              engineError
            );
          }
        }
